...
export async function runHttp(
  ...
  console.error("[fc-docs-mcp] 載入 " + notesCount() + " 篇 .md 文件");
  ...
  app.post("/mcp", async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.error("[fc-docs-mcp] 處理請求失敗:", err);
      if (!res.headersSent) {
        res.status(500).json({
          jsonrpc: "2.0",
  ...
  app.listen(port, () => {
    console.error("[fc-docs-mcp] HTTP server 已啟動:http://0.0.0.0:" + port + "/mcp");
    console.error("[fc-docs-mcp] 驗證:" + (authToken ? "已啟用 Bearer Token" : "未啟用 (公開存取)"));
  });
}
