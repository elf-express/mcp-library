...
### Dockhand(你的環境:GitOps + 接現有 gateway)

你已有一台 MCPJungle 在跑,所以**不要再起一個 gateway**——用 [`docker-compose.dockhand.yml`](./docker-compose.dockhand.yml) 只部署 `docs-mcp` + 一次性 `registrar`,把兩本書註冊進你現有的 gateway。**已沙盒實測**這條路(含 redeploy 冪等)。

1. **Dockhand → 新增 Git stack**:倉庫指向本 repo(HTTPS/SSH),compose 路徑填 `docs-mcp-server/docker-compose.dockhand.yml`。
2. **env 編輯器**(Dockhand 會存成 compose 旁的 `.env`):
   - `MCPJUNGLE_NETWORK`:你現有 gateway 的 Docker 網路**完整名**(其 compose 若沒設 `name:`,通常是 `<專案>_mcpjungl`;有設就是 `mcpjungl`)。
   - `REGISTRY_URL`:你現有 gateway 的 `http://<容器名>:8080`(預設 `http://mcpjungle-server:8080`)。
   - (選)`REGISTER_LIST`:預設 `sqlsugar fc`(只註冊書,因你 gateway 多半已有 filesystem 等)。
3. **開 webhook**:每次 `git push` → Dockhand 重拉、`docs-mcp` 重建、`registrar` 重跑(已註冊的自動略過)。

> registrar 內建**重試**(等 `docs-mcp` 開始監聽才註冊,避免 race)+ **冪等**(已註冊略過),所以 redeploy 安全。
> 想讓 Dockhand 連 gateway 一起管(不接現有的)?改用 [`docker-compose.mcpjungle.yml`](./docker-compose.mcpjungle.yml)(內含 gateway+postgres),env 填 `MCPJUNGLE_DATABASE_URL`。

### 建置 / 推送 image 到 ghcr.io

自 build 的 image 已命名為 `ghcr.io/elf-express/<name>:latest`(`docs-mcp-server` / `sqlsugar-notes-mcp` / `fc-docs-mcp`);`mcpjungle`/`postgres` 是官方 image 不推。手動推送:
...
推完別處即可 `docker pull ghcr.io/elf-express/docs-mcp-server:latest`;若部署端要「只 pull 不 build」,把該 compose 的 `docs-mcp` 服務移除 `build:` 即可。`build:` + registry `image:` 並存代表「在此 build、可推、也可被 pull」。
