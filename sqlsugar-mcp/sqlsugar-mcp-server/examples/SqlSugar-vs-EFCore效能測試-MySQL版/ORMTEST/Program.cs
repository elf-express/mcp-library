using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using PerformanceTest.Models;
using PerformanceTest.TestItems;
using SqlSugar;

namespace PerformanceTest
{
    class Program
    {

        /// <summary>
        /// 注意注意注意注意注意：分开测试比较公平,并且请在Realse模式下启动程序（SqlSugar直接引用的是项目）
        /// </summary>
        /// <param name="args"></param>
        static void Main(string[] args)
        {
            //配置好连接字符串和DbType自动建库建表建数据，需要SA高权限账户有建表权限
            InitData();

            new TestGetAll().Init(OrmType.EF);
            new TestGetAll().Init(OrmType.SqlSugar);

            //查询主键
            new TestGetById().Init(OrmType.EF);
            new TestGetById().Init(OrmType.SqlSugar);


            //查询分页
            new TestGetByPage().Init(OrmType.EF);
            new TestGetByPage().Init(OrmType.SqlSugar);


            //插入
            new TestInsert().Init(OrmType.EF);
            new TestInsert().Init(OrmType.SqlSugar);


            //插入集合100
            new TestInsertList().Init(OrmType.EF);
            new TestInsertList().Init(OrmType.SqlSugar);


            //插入集合1000
            new TestInsertList().Init(OrmType.EF);
            new TestInsertList().Init(OrmType.SqlSugar);
            Console.ReadKey();
        }

        private static void InitData()
        {
            SqlSugarClient conn = Config.GetSugarConn();
            conn.CurrentConnectionConfig.InitKeyType = InitKeyType.Attribute;
            conn.DbMaintenance.CreateDatabase();//创建库
            conn.CodeFirst.InitTables<Test>();
            List<Test> test = new List<Test>();
            if (conn.Queryable<Test>().Count() < 100000)
            {
                Console.WriteLine("初始化数据");
             
     

                for (int i = 0; i < 100000; i++)
                {
                    test.Add(new Test()
                    {
                        F_Bool = true,
                        F_Byte = 0,
                        F_DateTime = DateTime.Now,
                        //F_Decimal = 1,
                        F_Double = 11,
                        //F_Float = 11,
                        F_Guid = Guid.Empty,
                        F_String = "abc",
                        F_Int16 = 1,
                        F_Int32 = 1,
                        F_Int64 = 1

                    });
                }
                conn.Fastest<Test>().BulkCopy(test);
            }
          
            conn.CodeFirst.InitTables<Group>();
            // conn.DbMaintenance.TruncateTable<Group>();
            conn.CodeFirst.InitTables<User>();
        }

        enum DemoType
        {
            GetAll,
            GetById,
            GetSql,
            Insert,
            Like,
            OneToMany
        }
    }
}
